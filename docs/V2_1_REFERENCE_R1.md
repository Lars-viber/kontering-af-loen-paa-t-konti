# V2.1 – Reference R1

## 1. Formål og status

**Pre-release-korrektion efter manuel browsertest:** C afstemmer hele kontosaldi mod lønsystemets tælleværker, 2235 har ingen konstrueret ekstern tælleværkskontrol, og D er en intern driftskontosum.

Dette dokument er den autoritative, numeriske R1-reference for V2.1. Beløbene er hele kroner. `D` betyder debetsaldo, og `K` betyder kreditsaldo.

Referencen fastlægger:

- medarbejdergrundlag og satser,
- historik for januar-maj og juni-værdier,
- startsaldi,
- facit for B1-B9,
- lønsystemets synlige tælleværker,
- ultimosaldi og afstemningsfacit.

## 2. Medarbejdergrundlag

### Timelønnede

| Medarbejder | Timeløn | Trækprocent | Månedsfradrag | Jan | Feb | Mar | Apr | Maj | Jun |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| T1 | 240 | 36 % | 4.500 | 145 | 148 | 152 | 154 | 156 | 155 |
| T2 | 240 | 37 % | 4.750 | 155 | 160 | 163 | 162 | 164 | 165 |
| T3 | 230 | 38 % | 5.000 | 172 | 174 | 176 | 178 | 177 | 180 |
| T4 | 250 | 39 % | 5.250 | 172 | 170 | 176 | 174 | 178 | 180 |

### Månedslønnede

| Medarbejder | Månedsløn | Trækprocent | Månedsfradrag |
|---|---:|---:|---:|
| M1 | 42.000 | 37 % | 5.000 |
| M2 | 47.500 | 38 % | 5.250 |
| M3 | 52.500 | 39 % | 5.500 |
| M4 | 60.500 | 40 % | 5.750 |

### Faste satser

| Parameter | Værdi |
|---|---:|
| Medarbejderpension | 4 % |
| Arbejdsgiverpension | 8 % |
| Medarbejder-ATP pr. måned | 99 |
| Arbejdsgiver-ATP pr. måned | 198 |

## 3. Lønhistorik januar-juni

Historikken er kilde til de akkumulerede tælleværker. Den er en del af opgavens datagrundlag og må ikke behandles som et skjult facit.

### Timelønnede pr. måned

| Måned | Bruttoløn | Medarb. pension | Arbejdsgiverpension | Medarb. ATP | Arbejdsgiver-ATP | AM-grundlag | AM-bidrag | A-skat | Nettoløn |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| Januar | 154.560 | 6.182 | 12.365 | 396 | 792 | 147.982 | 11.838 | 43.848 | 92.296 |
| Februar | 156.440 | 6.258 | 12.516 | 396 | 792 | 149.786 | 11.983 | 44.451 | 93.352 |
| Marts | 160.080 | 6.403 | 12.806 | 396 | 792 | 153.281 | 12.262 | 45.663 | 95.356 |
| April | 160.280 | 6.411 | 12.822 | 396 | 792 | 153.473 | 12.278 | 45.718 | 95.477 |
| Maj | 162.010 | 6.480 | 12.961 | 396 | 792 | 155.134 | 12.411 | 46.295 | 96.428 |
| Juni | 163.200 | 6.528 | 13.056 | 396 | 792 | 156.276 | 12.502 | 46.702 | 97.072 |
| **Akkumuleret** | **956.570** | **38.262** | **76.526** | **2.376** | **4.752** | **915.932** | **73.274** | **272.677** | **569.981** |

### Månedslønnede pr. måned

De månedslønnedes værdier er ens i hver måned fra januar til juni.

| Måned | Bruttoløn | Medarb. pension | Arbejdsgiverpension | Medarb. ATP | Arbejdsgiver-ATP | AM-grundlag | AM-bidrag | A-skat | Nettoløn |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| Januar | 202.500 | 8.100 | 16.200 | 396 | 792 | 194.004 | 15.520 | 60.693 | 117.791 |
| Februar | 202.500 | 8.100 | 16.200 | 396 | 792 | 194.004 | 15.520 | 60.693 | 117.791 |
| Marts | 202.500 | 8.100 | 16.200 | 396 | 792 | 194.004 | 15.520 | 60.693 | 117.791 |
| April | 202.500 | 8.100 | 16.200 | 396 | 792 | 194.004 | 15.520 | 60.693 | 117.791 |
| Maj | 202.500 | 8.100 | 16.200 | 396 | 792 | 194.004 | 15.520 | 60.693 | 117.791 |
| Juni | 202.500 | 8.100 | 16.200 | 396 | 792 | 194.004 | 15.520 | 60.693 | 117.791 |
| **Akkumuleret** | **1.215.000** | **48.600** | **97.200** | **2.376** | **4.752** | **1.164.024** | **93.120** | **364.158** | **706.746** |

### Feriepenge for timelønnede

| Måned | Bruttoferiepenge | AM-bidrag | A-skat | Netto til FerieKonto |
|---|---:|---:|---:|---:|
| Januar | 19.320 | 1.546 | 6.682 | 11.092 |
| Februar | 19.556 | 1.564 | 6.760 | 11.232 |
| Marts | 20.010 | 1.601 | 6.917 | 11.492 |
| April | 20.036 | 1.603 | 6.924 | 11.509 |
| Maj | 20.252 | 1.620 | 7.000 | 11.632 |
| Juni | 20.400 | 1.632 | 7.052 | 11.716 |
| **Akkumuleret** | **119.574** | **9.566** | **41.335** | **68.673** |

### Juni-feriepenge pr. timelønnet

| Medarbejder | Bruttoferiepenge | AM-bidrag | A-skat | Netto til FerieKonto |
|---|---:|---:|---:|---:|
| T1 | 4.650 | 372 | 1.540 | 2.738 |
| T2 | 4.950 | 396 | 1.685 | 2.869 |
| T3 | 5.175 | 414 | 1.809 | 2.952 |
| T4 | 5.625 | 450 | 2.018 | 3.157 |
| **I alt** | **20.400** | **1.632** | **7.052** | **11.716** |

## 4. Startsaldi

### Drift pr. 31. maj

| Konto | Saldo |
|---:|---:|
| 2210 | 759.656 D |
| 2211 | 970.020 D |
| 2215 | 216.704 D |
| 2223 | 11.880 D |
| 2230 | 99.174 D |
| 2235 | 24.000 D |

### Balance pr. 1. juni

| Konto | Saldo |
|---:|---:|
| 5820 | 1.500.000 D |
| 6920 | 113.988 K |
| 6921 | 11.880 K |
| 6922 | 43.741 K |
| 6923 | 11.632 K |
| 6924 | 174.000 K |
| 6930 | 29.551 K |

## 5. Facit for B1-B9

### B1 – Betaling af A-skat og AM-bidrag vedr. maj

| Side | Konto | Beløb |
|---|---:|---:|
| Debet | 6920 | 113.988 |
| Debet | 6930 | 29.551 |
| Kredit | 5820 | 143.539 |

Kontrol: 143.539 = 143.539.

### B2 – Betaling af pension vedr. maj

| Side | Konto | Beløb |
|---|---:|---:|
| Debet | 6922 | 43.741 |
| Kredit | 5820 | 43.741 |

Kontrol: 43.741 = 43.741.

### B3 – Betaling til FerieKonto vedr. maj

| Side | Konto | Beløb |
|---|---:|---:|
| Debet | 6923 | 11.632 |
| Kredit | 5820 | 11.632 |

Kontrol: 11.632 = 11.632.

### B4 – Lønkørsel, timelønnede, juni

| Side | Konto | Beløb |
|---|---:|---:|
| Debet | 2210 | 156.276 |
| Debet | 2215 | 6.528 |
| Debet | 2223 | 396 |
| Kredit | 6920 | 46.702 |
| Kredit | 6930 | 12.502 |
| Kredit | 6922 | 6.528 |
| Kredit | 6921 | 396 |
| Kredit | 5820 | 97.072 |

Kontrol: 163.200 = 163.200.

### B5 – Arbejdsgiverbidrag, timelønnede, juni

| Side | Konto | Beløb |
|---|---:|---:|
| Debet | 2215 | 13.056 |
| Debet | 2223 | 792 |
| Kredit | 6922 | 13.056 |
| Kredit | 6921 | 792 |

Kontrol: 13.848 = 13.848.

### B6 – Feriepenge, timelønnede, juni

| Side | Konto | Beløb |
|---|---:|---:|
| Debet | 2230 | 20.400 |
| Kredit | 6930 | 1.632 |
| Kredit | 6920 | 7.052 |
| Kredit | 6923 | 11.716 |

Kontrol: 20.400 = 20.400.

### B7 – Lønkørsel, månedslønnede, juni

| Side | Konto | Beløb |
|---|---:|---:|
| Debet | 2211 | 194.004 |
| Debet | 2215 | 8.100 |
| Debet | 2223 | 396 |
| Kredit | 6920 | 60.693 |
| Kredit | 6930 | 15.520 |
| Kredit | 6922 | 8.100 |
| Kredit | 6921 | 396 |
| Kredit | 5820 | 117.791 |

Kontrol: 202.500 = 202.500.

### B8 – Arbejdsgiverbidrag, månedslønnede, juni

| Side | Konto | Beløb |
|---|---:|---:|
| Debet | 2215 | 16.200 |
| Debet | 2223 | 792 |
| Kredit | 6922 | 16.200 |
| Kredit | 6921 | 792 |

Kontrol: 16.992 = 16.992.

### B9 – Regulering af feriepengeforpligtelse pr. 30/6

| Side | Konto | Beløb |
|---|---:|---:|
| Debet | 2235 | 8.500 |
| Kredit | 6924 | 8.500 |

Kontrol: 8.500 = 8.500.

Før bogføring af B9 er den bogførte saldo 174.000 K, og den systemopgjorte saldo pr. 30/6 er 182.500 K. Reguleringen på 8.500 må ikke oplyses direkte før elevens bogføring. Konto 2235 har efter B9 en bogført saldo på 32.500 D, men beløbet er ikke et eksternt lønsystemtælleværk.

Samlet bilagskontrol: **9 af 9 bilag balancerer med difference 0 kr.**

## 6. Synlige tælleværker og kontroloplysninger

| Bilag | Tælleværk | Værdi |
|---|---|---:|
| B4 | Bruttoløn, timelønnede år til dato | 956.570 |
| B4 | Medarbejderpension, timelønnede år til dato | 38.262 |
| B4 | Medarbejder-ATP, timelønnede år til dato | 2.376 |
| B5 | Arbejdsgiverpension, timelønnede år til dato | 76.526 |
| B5 | Arbejdsgiver-ATP, timelønnede år til dato | 4.752 |
| B6 | Feriepenge, timelønnede år til dato | 119.574 |
| B7 | Bruttoløn, månedslønnede år til dato | 1.215.000 |
| B7 | Medarbejderpension, månedslønnede år til dato | 48.600 |
| B7 | Medarbejder-ATP, månedslønnede år til dato | 2.376 |
| B8 | Arbejdsgiverpension, månedslønnede år til dato | 97.200 |
| B8 | Arbejdsgiver-ATP, månedslønnede år til dato | 4.752 |
| B9 | Feriepengeforpligtelse ifølge lønsystem | 182.500 |

Eksterne kontroloplysninger om åbne forpligtelser pr. 30. juni:

| Forpligtelse | Kontroltal |
|---|---:|
| A-skat | 114.447 |
| AM-bidrag | 29.654 |
| Pension | 43.884 |
| ATP | 14.256 |
| FerieKonto | 11.716 |
| Feriepengeforpligtelse | 182.500 |

## 7. Ultimosaldi pr. 30. juni

### Drift

| Konto | Ultimosaldo |
|---:|---:|
| 2210 | 915.932 D |
| 2211 | 1.164.024 D |
| 2215 | 260.588 D |
| 2223 | 14.256 D |
| 2230 | 119.574 D |
| 2235 | 32.500 D |
| **I alt** | **2.506.874 D** |

### Balance

| Konto | Ultimosaldo |
|---:|---:|
| 5820 | 1.086.225 D |
| 6920 | 114.447 K |
| 6930 | 29.654 K |
| 6922 | 43.884 K |
| 6921 | 14.256 K |
| 6923 | 11.716 K |
| 6924 | 182.500 K |

Dette er V2.1''s final state. De åbne kreditposter er tilsigtede juni-forpligtelser, fordi forløbet stopper pr. 30/6 før afregning af juni.

## 8. Afstemningsfacit A-E

### A – Timelønnede

`915.932 + 38.262 + 2.376 = 956.570`

Kontroltal: 956.570. Difference: **0**.

### B – Månedslønnede

`1.164.024 + 48.600 + 2.376 = 1.215.000`

Kontroltal: 1.215.000. Difference: **0**.

### C – Afstem pension, ATP og feriepenge

| Område | Fra bogføringen / T-konto | Fra lønsystemets tælleværker | Sum | Difference |
|---|---:|---|---:|---:|
| Pension | 2215: 260.588 | 38.262 + 76.526 + 48.600 + 97.200 | 260.588 | 0 |
| ATP | 2223: 14.256 | 2.376 + 4.752 + 2.376 + 4.752 | 14.256 | 0 |
| Feriepenge – timelønnede | 2230: 119.574 | Bruttoferiepenge ÅTD: 119.574 | 119.574 | 0 |

Eleven indtaster bogføringssaldoen og alle fire pensions-/ATP-tælleværker samt bruttoferiepengetælleværket. Summerne afledes. Konto 2235 indgår ikke i C.

### D – Intern kontrol af samlede lønrelaterede omkostninger

`915.932 + 1.164.024 + 260.588 + 14.256 + 119.574 + 32.500 = 2.506.874`

D er beregnet fra bogføringens seks driftskonti 2210, 2211, 2215, 2223, 2230 og 2235. Der findes ingen ekstern tællerværkssum eller difference i D.
### E – Åbne forpligtelser

| Konto | Bogført saldo | Ekstern kontrol | Difference |
|---:|---:|---:|---:|
| 6920 | 114.447 | 114.447 | 0 |
| 6930 | 29.654 | 29.654 | 0 |
| 6922 | 43.884 | 43.884 | 0 |
| 6921 | 14.256 | 14.256 | 0 |
| 6923 | 11.716 | 11.716 | 0 |
| 6924 | 182.500 | 182.500 | 0 |

Alle checkpointets krævede differencer er **0 kr.**

## 9. Historisk afgrænsning

**SUPERSEDED IN V2.1:** B10-B13 findes ikke i R1-facittet eller den aktive V2.1-kontrakt.

**SUPERSEDED IN V2.1:** En separat `finalControl` eller slutkontrol findes ikke i R1. A-E er den samlede faglige slutafstemning.
